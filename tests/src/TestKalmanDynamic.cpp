...
static void gyro_test(const Matrix<fp, 3, 1> &angle_increment, const char *filename = nullptr) {
    ...
    for (int i = 0; i < NUM_TESTS; i++) {
        ...
        expect_fuzzy_eq(estimate.angular_velocity[0], angle_increment[0] * time_increment, kalman_gyro_cov, std::max(kalman_gyro_cov, 0.05f));
        expect_fuzzy_eq(estimate.angular_velocity[1], angle_increment[1] * time_increment, kalman_gyro_cov, std::max(kalman_gyro_cov, 0.05f));
        expect_fuzzy_eq(estimate.angular_velocity[2], angle_increment[2] * time_increment, kalman_gyro_cov, std::max(kalman_gyro_cov, 0.05f));
