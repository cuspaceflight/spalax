...
inline float mat2x2_det(float mat[2][2]) {
	return mat[0][0] * mat[1][1] - mat[0][1] * mat[1][0];
}

inline void mat2x2_inv(float mat[2][2], float out[2][2]) {
	out[0][0] = mat[1][1];
	out[1][1] = mat[0][0];
	out[0][1] = -mat[0][1];
	out[1][0] = -mat[1][0];
...
inline float mat3x3_det(float mat[3][3]) {
	return mat[0][0] * (mat[1][1] * mat[2][2] - mat[2][1] * mat[1][2])
		- mat[0][1] * (mat[1][0] * mat[2][2] - mat[1][2] * mat[2][0])
		+ mat[0][2] * (mat[1][0] * mat[2][1] - mat[1][1] * mat[2][0]);
}

inline void mat3x3_inv(float m[3][3], float out[3][3]) {
...
inline void mat3x3_mult(float a[3][3], float b[3][3], float out[3][3]) {
...
inline float mat4x4_det(float m[4][4]) {
...
inline void mat4x4_inv(float m[4][4], float out[4][4]) {
...
inline void mat4x4_mult(float a[4][4], float b[4][4], float out[4][4]) {
...
inline float vector_dot(const float a[3], const float b[3]) {
	return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
}

inline void vector_cross(const float a[3], const float b[3], float out[3]) {
	out[0] = a[1] * b[2] - a[2] * b[1];
	out[1] = a[2] * b[0] - a[0] * b[2];
	out[2] = a[0] * b[1] - a[1] * b[0];
}

inline float vector_cross_mag(const float a[3], const float b[3]) {
...
inline void axis_angle_to_euler(const float axis[3], const float angle, float out[3]) {
	float s = sinf(angle);
	float c = cosf(angle);
	float t = 1 - c;
...
inline void quat_to_euler(const float q[4], float xyz[3]) {
...
inline void axis_angle_to_quat(const float axis[3], const float angle, float q[4]) {
...
inline void quaternion_to_rodrigues(const float q[4], float mrp[3]) {
    mrp[0] = q[0] / (1 + q[3]);
    mrp[1] = q[1] / (1 + q[3]);
    mrp[2] = q[2] / (1 + q[3]);
}

inline void rodrigues_to_quaternion(const float mrp[3], float q[4]) {
    //
...
inline void quat_mult(const float q1[4], const float q2[4], float out[4]) {
...
inline float vector_mag(const float v[3]) {
    return sqrtf(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
}

inline void quat_rotate(const float q[4], const float v[3], float out[3]) {
    float uv[3];
    float uuv[3];
...
inline void apply_q(const float q[4], const float v[3], float out[3]) {
